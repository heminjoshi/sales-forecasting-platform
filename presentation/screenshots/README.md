...
Open `http://localhost:8080` (tenant `tenant_a`, mode **forecast**, window **month**, channel **all**, k **10**).
...
2. **`dashboard-degraded.png`** — the resilience moment. Run the wipe from
   `../demo-script.md` §3 (TRUNCATE `serving_rows, serving_active_version` + `redis-cli INCR tenantver:tenant_a`),
